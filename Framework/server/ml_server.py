                ...
                for request in request_iterator:
                    ...
                    with self.lock:
                        ...
                        if should_continue(list(self.round_buffer.queue)):
                            avg_vector = compute_average_vector(list(self.round_buffer.queue))
                            broadcast_result(avg_vector, self.client_queues)
                            end_time = time.time()
                            print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] [Server] Round {self.round_id} completed in {end_time - start_time:.2f} seconds.")
