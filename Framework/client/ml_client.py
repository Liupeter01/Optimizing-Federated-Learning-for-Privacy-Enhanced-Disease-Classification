...
def generate_requests(client_id, model_version="1.0.0"):
    try:
        while not send_queue.empty():
            ...
            print(f"[Client] Sent vector from Client {client_id}, Version {
                  model_version}, Size: {len(vector)}, Vec: {vector[:10]} (First 10 elements)")
        ...
        try:
            ...
            if response.status_code != 0:
                print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')
                          }] [Client] Server Error: {response.error_message}")
                break

            print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] [Client] Round {round_num +
                  1}: Received merged vector of size {len(response.merged_vector)} in {end_time - start_time:.2f} seconds.")
            local_vector = handle_merged_vector(
                response.merged_vector, local_vector)
            ...
            print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')
                      }] [Client] Server has completed the stream.")
            break
        except Exception as e:
            print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')
                      }] [Client] Error during round {round_num + 1}: {e}")
...
def run(client_id, model_version="1.0.0", server_address="localhost:50051", max_rounds=4):
    ...
    try:
        ...
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')
                  }] [Client] Failed to connect to server: {e}")
        return

    try:
        response_iterator = stub.FederatedAveraging(
            generate_requests(client_id, model_version))
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')
                  }] [Client] Started Federated Averaging.")
    except Exception as e:
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')
                  }] [Client] Error starting gRPC stream: {e}")
        return

    # Start response thread
    response_thread = threading.Thread(target=receive_responses, args=(
        response_iterator, local_vector, max_rounds))
    response_thread.start()
    response_thread.join()

    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')
              }] [Client] Federated Averaging completed.")
